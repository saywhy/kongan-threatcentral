...
myApp.controller("AlertLoopholeCtrl", function ($scope, $http, $filter, $sce) {
    $scope.init = function () {
        ...
        $scope.select_ul = false;
        $scope.select_ul_poc = false;
        //   $scope.select_ul_company = false;
        $("html").click(function (e) {
            if ($(e.target).parents().hasClass("sun_select_box")) {} else {
    ...
    // sun
    $scope.focus_select = function () {
        $scope.select_ul = true;
    }
    $scope.choose_item = function (item) {
        $scope.loop_serch_data.company = item.type;
        $scope.select_ul = false;
    }
    $scope.focus_select_poc = function () {
        $scope.select_ul_poc = true;
    }
    $scope.choose_item_poc = function (item) {
        $scope.loop_serch_data.poc = item.type;
        $scope.select_ul_poc = false;
    }
    // sun
    ...
    $scope.loop_download = function () {
        ...
        var poc_data = ''
        var company_data = ''
        if ($scope.loop_serch_data.poc != '请选择有无POC') {
            poc_data = $scope.loop_serch_data.poc
        }
        if ($scope.loop_serch_data.company != '请选择所属') {
            company_data = $scope.loop_serch_data.company
        }
        ...
        input6.attr("value", company_data);
        form.append(input6);

        var input7 = $("<input>");
        input7.attr("type", "hidden");
        input7.attr("name", "poc");
        input7.attr("value", poc_data);
        form.append(input7);
        form.submit(); //表单提交
    };
