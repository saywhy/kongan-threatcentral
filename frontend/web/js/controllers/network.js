...
myApp.controller("netCtrl", function ($scope, $http) {
    ...
    $scope.choose_item = function (item) {
        $scope.select_net_name = item.type
        $scope.select_ul = false;
    }
    $scope.focus_select_gain = function () {
        $scope.select_ul_gain = true;
    }
    $scope.choose_item_gain = function (item) {
        $scope.select_gain_type = item.type
        $scope.select_ul_gain = false;
    }
                ...
                if (rsp.data.data.result) {
                    ...
                    if (name == "save") {
                        ...
                        if (
                            ...
                            $scope.select_gain_type = "自动获取";
                        }
                        if (
                            $scope.network_list[$scope.select_model[0].num].BOOTPROTO ==
                            "static"
                        ) {
                            $scope.select_gain_type = "手动设置";
                        }
                        $scope.network = {
